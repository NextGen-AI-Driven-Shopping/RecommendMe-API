...
def _build_session_title(query: str, max_len: int = 48) -> str:
    ...
    # Prefer a word boundary near the limit to avoid abrupt cuts.
    boundary = cleaned.rfind(" ", 0, max_len)
    if boundary < max_len * 0.6:
        boundary = max_len
    return f"{cleaned[:boundary].rstrip()}..."
    ...
    _store_session_snapshot(
        session_id,
        {
            **existing_session,
            ...
            "title": existing_session.get("title") or _build_session_title(clean_query),
